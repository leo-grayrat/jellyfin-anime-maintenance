...
$Headers = @{
    Authorization = "MediaBrowser Client=`"library-export`", Device=`"PowerShell`", DeviceId=`"library-export`", Version=`"1.0`", Token=`"$ApiKey`""
}
...
do {
    ...
    Write-Host "Exported $StartIndex / $($Response.TotalRecordCount)"

} while (
    $PageItems.Count -gt 0 -and
    $StartIndex -lt $Response.TotalRecordCount
)

$Libraries = Invoke-RestMethod `
    -Uri "$Server/Library/VirtualFolders" `
    -Headers $Headers
