...
## 2026-08-16 远程访问实机排查：公网 IPv6 已通，TC7102 防火墙成为唯一已确认阻断点

媒体库整理基本完成后，项目回到最初目标：让动漫社成员从外网访问这台 Jellyfin。

先尝试 Tailscale，但移动端会占用系统 VPN 通道，与用户原有代理互斥；而且本轮又遇到“能 ping、网页打不开”等额外网络问题，因此暂时不把“要求所有社员安装 Tailscale”作为默认方案，也暂不继续排查该支线。

随后转向直接公网访问测试。家中 Windows 主机当前 WLAN 获得：

- 私网 IPv4：`192.168.2.x`；
- 全球 IPv6：`2408:824e:5100:16f1:...`。

Jellyfin 开启 IPv6 后，重启服务，`netstat -ano | findstr :8096` 确认同时出现：

```text
TCP 0.0.0.0:8096 LISTENING
TCP [::]:8096    LISTENING
```

随后做分层实机测试：

1. 同一 Wi-Fi 下，手机可以直接通过 `http://[2408:...]:8096` 打开 Jellyfin；
2. 手机关闭 Wi-Fi、改用移动网络后，访问同一 IPv6 地址失败；
3. Windows 中未找到既有 8096 入站规则；
4. 临时显式添加 `TCP 8096 / Inbound / Allow / Profile Any` 的 Windows 防火墙规则后，移动网络访问仍失败，因此 Windows 防火墙不是本轮阻断原因；
5. 家用路由器确认为中国电信定制华为 TC7102；后台只有 `安全设置 → 防火墙` 总开关，未看到可针对 IPv6 单独配置目标主机/端口的 WAN→LAN 允许规则；
6. **临时关闭 TC7102 防火墙后，手机移动网络立即可以通过公网 IPv6 打开 Jellyfin；重新开启后恢复阻断。**
...
整条链路本身可用。当前唯一已经通过开/关实验明确定位的阻断点是 **TC7102 的公网 IPv6 入站防火墙**。

同时确认两条边界：

- `安全设置 → NAT 服务` 解决的是 IPv4 NAT/端口映射，不会绕过当前 IPv6 入站防火墙；
- 永久关闭 TC7102 总防火墙会让家庭中所有拥有全球 IPv6 的设备失去路由器统一的公网入站保护，因此只适合作为诊断，不作为已经接受的最终方案。

后续若继续远程访问部署，应优先评估“不更换路由器、不永久关闭总防火墙、普通社员不安装 VPN 客户端”的可行方案，例如：运营商提供公网 IPv4 后仅做必要端口映射，或由本机主动向公网中继/VPS 建立出站隧道，再由公网入口提供 HTTPS。当前阶段先记录事实，不把任何一种方案视为已决定。